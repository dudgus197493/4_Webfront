...
document.getElementById("check2").addEventListener("click", function(){
    ...
    // ^(캐럿) : 문자열의 시작을 의미
    const regEx2 = /^group/;    // 문자열이 group이라는 단어로 시작하는가?
    ...
    // $(달러) : 문자열의 끝을 의미
    const regEx4 = /team$/; // temp으로 끝나는 문자열
    ...
    div1.innerHTML += "/^[ABCD]-team$/  ,  Asdf-team : " + regEx5.test("Asdf-team") + "<hr>"; // f
...
document.getElementById("inputName").addEventListener("keyup", function(){
    ...
    // - 단자음/모음 경우는 제외
    const regEx = /^[가-힣a-z0-9]{2,6}$/;

    const result1 = document.getElementById("result1");

    // 유효성 검사
    // 여기서 this == document.getElementById("inputName")
    if(regEx.test(this.value)) {  // 유효한 형식
