...
document.getElementById("btn1").addEventListener("click",function(){
    ...
    // 요소.parentNode : 요소의 부모 노드를 탐색해서 반환 (부모노드는 단 한개만 가질 수 있음)
    
    // 요소.childNodes : 요소의 자식 노드를 모두 반환(NodeList 형태)
    // 요소.firstChild : 첫번째 자식노드 탐색해서 반환
    // 요소.lastChild : 마지막 자식 노드를 탐색해서 반환
    
    // 요소.childNode[index] : index 번째 자식 노드를 탐색해서 반환

    // 형제
    // 요소.previousSilbling : 요소의 이전 형제 노드를 반환
    ...
    // #li1의 부모의 배경을 orange로 변경 (노드 탐색 활용 방법)
    ...
    // #test의 자식 노드중 5번 인덱스 노드 찾기
    console.log(test.childNodes[5]);

    // #test의 자식 노드중 5번 인덱스 노드의 마지막 자식으로 "ZZZ" 추가하기
    test.childNodes[5].append("ZZZ");
    
    // #test의 자식 노드중 5번 인덱스 노드의 이전 형제 노드
    console.log(test.childNodes[5].previousSibling);

    // #test의 자식 노드중 5번 인덱스 노드의 다음 형제 노드
    console.log(test.childNodes[5].nextSibling);


    // 노드 탐색 구문 연속 작성
    // #test의 자식 노드중 5번 인덱스 노드의 이전 형제 노드의 이전 형제 노드
